...
open class IMSessionViewController: BaseViewController, UITableViewDataSource, UITableViewDelegate,
    IMSessionCellOperator
{

    private var containerView = UIView()
    private var sessionTableView = UITableView()
    private var sessions: [Session] = Array()
    ...
    private func onSessionUpdate(_ session: Session) {
        ...
        if oldPos >= 0 && oldPos < self.sessions.count {
            ...
            if oldPos == insertPos {
                ...
                if let cell = tableView.cellForRow(at: IndexPath.init(row: insertPos, section: 0))
                    as? IMBaseSessionCell
                {
                    cell.updateSession(session)
                } else {
                    tableView.reloadRows(
                        at: [IndexPath.init(row: insertPos, section: 0)], with: .none)
                }
            } else {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let distance =
            self.sessionTableView.contentSize.height - self.sessionTableView.contentOffset.y
        if h > distance {
            self.loadSessions()
        }
    }
    ...
    func registerSessionEvent() {
        ...
        SwiftEventBus.onMainThread(
            self, name: IMEvent.SessionDelete.rawValue,
            handler: { [weak self] result in
                guard let session = result?.object as? Session else {
                    return
                }
    ...
    public func tableView(
        ...
        let mute = UIContextualAction(
            style: .normal, title: silenceText,
            handler: { [weak self] action, view, completionHandler in
                guard let sf = self else {
                    return
                }

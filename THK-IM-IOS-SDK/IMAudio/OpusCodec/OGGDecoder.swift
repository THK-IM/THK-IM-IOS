...
class OGGDecoder {
    ...
    private func extractPacket(_ streamState: inout ogg_stream_state, _ packet: inout ogg_packet) throws {
        ...
        while ogg_stream_packetout(&streamState, &packet) == 1 {
            ...
            let decodeCount = opus_decode(decoder, packet.packet, Int32(packet.bytes), pcmDataBuffer, 8196, 0)
            if (decodeCount > 0) {
                let bufferPointer = UnsafeBufferPointer(start: pcmDataBuffer, count: Int(decodeCount))

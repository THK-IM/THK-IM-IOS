import AVFoundation
//
//  AVAudioEngineRTCAudioDevice.swift
//  THK-IM-IOS
//
...
final class AVAudioEngineRTCAudioDevice: NSObject {
    ...
    private lazy var audioInputFormat = AVAudioFormat(
    ...
    private lazy var audioOutputFormat = AVAudioFormat(
        commonFormat: .pcmFormatInt16,
        sampleRate: audioSession.sampleRate,
    ...
    private func updateEngine() {
        guard let delegate = delegate,
            ...
            print(
                "Audio Engine must be stopped: shouldPla=\(shouldPlay), shouldRecord=\(shouldRecord), isInterrupted=\(isInterrupted)"
            )
        ...
        if let audioEngine = audioEngine,
            !audioEngine.isInputOutputSampleRatesNativeFor(audioSession: audioSession)
        {
        ...
        if let audioEngine = audioEngine,
            audioEngine.inputNode.isVoiceProcessingEnabled != useVoiceProcessingAudioUnit
        {
        ...
        if let engine = self.audioEngine {
            ...
            if audioEngine.outputNode.isVoiceProcessingEnabled != useVoiceProcessingAudioUnit {
                do {
                    ...
                    try audioEngine.outputNode.setVoiceProcessingEnabled(
                        useVoiceProcessingAudioUnit)
                } catch let e {
        ...
        if ioAudioUnit.isInputEnabled != shouldRecord || ioAudioUnit.isOutputEnabled != shouldPlay {
            if audioEngine.isRunning {
                measureTime(
                    label: "AVAudioEngine stop (to enable/disable AUAudioUnit output/input)"
                ) {
                    audioEngine.stop()
                }
            }
        ...
        if shouldRecord {
            if audioSinkNode == nil {
                measureTime(label: "Add AVAudioSinkNode") {
                    ...
                    let customRenderBlock: RTCAudioDeviceRenderRecordedDataBlock = {
                        actionFlags, timestamp, inputBusNumber, frameCount, abl, renderContext in
                        let (converter, inputData) = renderContext!.assumingMemoryBound(
                            to: (
                                Unmanaged<SimpleAudioConverter>,
                                UnsafeMutablePointer<AudioBufferList>
                            ).self
                        ).pointee
                        return converter.takeUnretainedValue().convert(
                            framesCount: frameCount, from: inputData, to: abl)
                    }

                    let audioSink = AVAudioSinkNode(receiverBlock: {
                        (timestamp, framesCount, inputData) -> OSStatus in
                        var flags: AudioUnitRenderActionFlags = []
                        var renderContext = (Unmanaged.passUnretained(converter), inputData)
                        return deliverRecordedData(
                            &flags, timestamp, 1, framesCount, nil, &renderContext,
                            customRenderBlock)
                    })
        ...
        if shouldPlay {
            if audioSourceNode == nil {
                measureTime(label: "Add AVAudioSourceNode") {
                    ...
                    audioEngine.connect(
                        audioEngine.mainMixerNode, to: audioEngine.outputNode, format: outputFormat)
                    ...
                    measureTime(label: "Connect AVAudioSourceNode") {
                        audioEngine.connect(
                            audioSource, to: audioEngine.mainMixerNode, format: outputFormat)
                    }
...
//                    audioEngine.connect(
//                        backgroundPlayer, to: audioEngine.mainMixerNode, format: nil)
//                    if !backgroundPlayer.isPlaying {
//                        backgroundPlayer.play()
//                        backgroundPlayer.scheduleBuffer(
//                            backgroundSound, at: nil, options: [.loops], completionHandler: nil)
//                    }
//                }
//            }
        }
...
extension AVAudioEngineRTCAudioDevice: RTCAudioDevice {
    ...
    var deviceInputSampleRate: Double {
        guard let sampleRate = audioInputFormat?.sampleRate, sampleRate > 0 else {
            DDLogError("LiveRTCEngine deviceInputSampleRate 1: \(audioSession.sampleRate)")
            return audioSession.sampleRate
        }
        DDLogError("LiveRTCEngine deviceInputSampleRate 2: \(sampleRate)")
        return sampleRate
    }

    var deviceOutputSampleRate: Double {
        guard let sampleRate = audioOutputFormat?.sampleRate, sampleRate > 0 else {
            DDLogError("LiveRTCEngine deviceOutputSampleRate 1: \(audioSession.sampleRate)")
            return audioSession.sampleRate
        }
        DDLogError("LiveRTCEngine deviceOutputSampleRate 2: \(sampleRate)")
        return sampleRate
    }
    ...
    var inputNumberOfChannels: Int {
        guard let channelCount = audioInputFormat?.channelCount, channelCount > 0 else {
            DDLogError("LiveRTCEngine inputNumberOfChannels 1: \(audioSession.inputNumberOfChannels)")
            return min(2, audioSession.inputNumberOfChannels)
        }
        DDLogError("LiveRTCEngine inputNumberOfChannels 2: \(channelCount)")
        return Int(channelCount)
    }

    var outputNumberOfChannels: Int {
        guard let channelCount = audioOutputFormat?.channelCount, channelCount > 0 else {
            DDLogError("LiveRTCEngine outputNumberOfChannels 1: \(audioSession.outputNumberOfChannels)")
            return min(2, audioSession.outputNumberOfChannels)
        }
        DDLogError("LiveRTCEngine outputNumberOfChannels 2: \(channelCount)")
        return Int(channelCount)
    }

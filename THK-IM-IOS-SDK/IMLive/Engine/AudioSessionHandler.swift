...
extension AudioSessionHandler {
    func subscribeAudioSessionNotifications() -> [Any] {
        ...
        let interruptionNotificationSubscribtion = center.addObserver(
            ...
            guard let type = notification.userInfo?[AVAudioSessionInterruptionTypeKey] as? NSNumber,
                let interruptionType: AVAudioSession.InterruptionType = .init(
                    rawValue: type.uintValue)
            else {
            ...
            case .began:
                ...
                if #available(iOS 14.5, *) {
                    if let rawReason = notification.userInfo?[AVAudioSessionInterruptionReasonKey]
                        as? NSNumber,
                        let reason: AVAudioSession.InterruptionReason = .init(
                            rawValue: rawReason.uintValue)
                    {
                        applicationWasSuspended = reason == .appWasSuspended
                    }
                } else {
                    if let wasSuspended = notification.userInfo?[
                        AVAudioSessionInterruptionWasSuspendedKey] as? NSNumber,
                        wasSuspended.boolValue
                    {
            ...
            case .ended:
                ...
                if let type = notification.userInfo?[AVAudioSessionInterruptionOptionKey]
                    as? NSNumber
                {
                    let interruptionOptions: AVAudioSession.InterruptionOptions = .init(
                        rawValue: type.uintValue)
        ...
        let routeChangeNotificationSubscribtion = center.addObserver(
            ...
            print(
                "\(AVAudioSession.routeChangeNotification): \(notification) -> \(self.audioSession.describedState)"
            )
            self.handleAudioRouteChange()
        }

        let mediaServicesWereLostNotificationSubscribtion = center.addObserver(
        ...
        let mediaServicesWereResetNotificationSubscribtion = center.addObserver(
    ...
    func configureStereoRecording() {
        ...
        guard let preferredInput = audioSession.preferredInput,
            ...
            let supportedPolarPatterns = frontStereo.supportedPolarPatterns
        else {

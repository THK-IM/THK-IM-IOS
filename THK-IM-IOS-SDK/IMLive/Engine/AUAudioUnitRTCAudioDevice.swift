...
final class AUAudioUnitRTCAudioDevice: NSObject {
    ...
    internal lazy var inputLatency = audioSession.inputLatency {
        didSet {
    ...
    internal lazy var outputLatency = audioSession.outputLatency {
        didSet {
    ...
    private func updateAudioUnit() {
        ...
        if shouldRecord {
            ...
            if outputBus.format != rtcRecordFormat {
                stopAndUnitializeAudioUnit(
                    "Stop to update recording format of audioUnit.outputBusses[\(bus)]")
                measureTime(label: "Update recording format of audioUnit.outputBusses[\(bus)]") {
                    do {
                        ...
                        print(
                            "Failed update audioUnit.outputBusses[\(bus)].format of audio unit: \(e)"
                        )
                        return
                    }
                }
            ...
            measureTime(label: "AVAudioUnit define inputHandler") {
                ...
                audioUnit.inputHandler = { actionFlags, timestamp, frameCount, inputBusNumber in
                    let status = deliverRecordedData(
                        actionFlags, timestamp, inputBusNumber, frameCount, nil, nil,
                        customRenderBlock)
                    if status != noErr {
            ...
            if inputBus.format != rtcPlayFormat {
                ...
                measureTime(label: "Update playout format of audioUnit.inputBusses[\(bus)]") {
                    do {
                        ...
                        print(
                            "Failed update audioUnit.inputBusses[\(bus)].format of audio unit: \(e)"
                        )
                        return
                    }
                }
            ...
            if audioUnit.outputProvider == nil {
                measureTime(label: "AVAudioUnit define outputProvider") {
                    ...
                    audioUnit.outputProvider = {
                        (actionFlags, timestamp, frameCount, inputBusNumber, inputData)
...
extension AUAudioUnitRTCAudioDevice: RTCAudioDevice {
    ...
    func initialize(with delegate: RTCAudioDeviceDelegate) -> Bool {
        ...
        let description = AudioComponentDescription(
            ...
            componentSubType: audioSession.supportsVoiceProcessing

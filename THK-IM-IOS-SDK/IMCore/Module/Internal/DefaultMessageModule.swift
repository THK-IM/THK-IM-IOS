...
open class DefaultMessageModule: MessageModule {
    ...
    private let epoch: Int64 = 1_288_834_974_657
    ...
    open func batchProcessMessages(_ messages: [Message], _ ack: Bool = true) throws {
        ...
        for (sid, msgs) in sessionMsgs {
            ...
            if !referMsgIds.isEmpty {
                let referMsgs = try? IMCoreManager.shared.database.messageDao().findByMsgIds(
                    referMsgIds, sid)
                if referMsgs != nil {
                    for m in msgs {
            ...
            // 更新每个session的最后一条消息
            let lastMsg = try IMCoreManager.shared.database.messageDao().findLastMessageBySessionId(
                sid)
            if lastMsg != nil {
                ...
                do {
                    for m in messages {
                        if m.cTime <= session.mTime {
                            m.operateStatus =
                                m.operateStatus | MsgOperateStatus.ClientRead.rawValue
                    ...
                    if !messages.isEmpty {
                        ...
                        if lastTime != nil {
                            try IMCoreManager.shared.database.sessionDao().updateMsgSyncTime(
                                session.id, lastTime!)
    ...
    open func syncSuperGroupMessages() {
        Observable.just("")
            .flatMap({ (it) -> Observable<[Session]?> in
                let sessions = try? IMCoreManager.shared.database.sessionDao().findAll(
                    SessionType.SuperGroup.rawValue)
                ...
                if let sf = self {
                    if sessions != nil {
                        for session in sessions! {
                            if session.deleted == 0 && session.id > 0
    ...
    open func getSession(_ entityId: Int64, _ type: Int) -> Observable<Session> {
        return Observable.create({ observer -> Disposable in
            do {
                var session = try IMCoreManager.shared.database.sessionDao().findByEntityId(
                    entityId, type)
                if session == nil {
                    session = Session.emptySession()
                }
            ...
            if session.id > 0 {
                ...
                return IMCoreManager.shared.api.queryUserSession(
                    IMCoreManager.shared.uId, entityId, type
                )
                .flatMap({ session in
                    if session.id > 0 && session.deleted == 0 {
                        try? IMCoreManager.shared.database.sessionDao().insertOrIgnore([session])
    ...
    open func queryLocalSessions(_ parentId: Int64, _ count: Int, _ mTime: Int64) -> Observable<
        [Session]
    > {
        return Observable.create({ observer -> Disposable in
            do {
                let sessions = try IMCoreManager.shared.database.sessionDao().findByParentId(
                    parentId, count, mTime)
                if sessions != nil {
    ...
    open func queryLocalMessages(
        ...
        return Observable.create({ observer -> Disposable in
            ...
            let messages = IMCoreManager.shared.database.messageDao().findByTimeRange(
                sessionId, startTime, endTime, count, excludeMsgId)
    ...
    open func onNewMessage(_ msg: Message) {
        DDLogInfo(
            "MessageModule onNewMessage \(Thread.current.isMainThread ? "main": "io") \(msg.msgId) \(msg.type) \(msg.content ?? "")"
        )
    ...
    open func sendMessage(
        _ sessionId: Int64, _ type: Int, _ body: Codable?, _ data: Codable?,
        _ atUser: String? = nil,
        ...
        do {
            try IMCoreManager.shared.database
                ...
                .updateOperationStatus(
                    sessionId, msgIds.compactMap({ $0 }), MsgOperateStatus.Ack.rawValue)
        } catch {
    ...
    open func deleteLocalMessages(_ messages: [Message]) -> Observable<Void> {
        return Observable.create({ [weak self] observer -> Disposable in
            do {
                ...
                for sessionId in sessionIds {
                    if let lastMsg = try? IMCoreManager.shared.database.messageDao()
                        .findLastMessageBySessionId(sessionId)
                    {
    ...
    open func deleteMessages(_ sessionId: Int64, _ messages: [Message], _ deleteServer: Bool)
        -> Observable<Void>
    {
        if deleteServer {
            var ids = Set<Int64>()
    ...
    open func deleteServerSession(_ session: Session) -> Observable<Void> {
        return IMCoreManager.shared.api.deleteUserSession(
    ...
    open func updateServerSession(_ session: Session) -> Observable<Void> {
        return IMCoreManager.shared.api.updateUserSession(
            IMCoreManager.shared.uId, session: session)
    }
    ...
    > {
        ...
        }).flatMap({ mTime -> Observable<[SessionMember]> in
            return IMCoreManager.shared.api.queryLatestSessionMembers(sessionId, mTime, nil, count)
                .flatMap({ members -> Observable<[SessionMember]> in
                    ...
                    if !members.isEmpty {
                        ...
                        try IMCoreManager.shared.database.sessionDao().updateMemberSyncTime(
                            sessionId, lastMTime)
                    }

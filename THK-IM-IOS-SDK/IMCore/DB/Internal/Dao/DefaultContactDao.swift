...
open class DefaultContactDao: ContactDao {
    ...
    public func findAll() -> [Contact] {
        let contacts: [Contact]? = try? self.database?.getObjects(
    ...
    public func findByUserIds(_ ids: [Int64]) -> [Contact] {
        let contacts: [Contact]? = try? self.database?.getObjects(
            fromTable: self.tableName,
    ...
    public func findByRelation(_ relation: Int) -> [Contact] {
        let contacts: [Contact]? = try? self.database?.getObjects(
            fromTable: self.tableName,

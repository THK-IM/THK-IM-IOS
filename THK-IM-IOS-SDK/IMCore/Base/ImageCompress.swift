...
public class ImageCompressor {
    ...
    public static func compressImageFile(_ srcPath: String, _ desPath: String, _ options: Options)
        throws
    {
        ...
        if srcData.detectImageType() == "gif" {
            ...
            let width =
                CGFloat(image.size.width) * (CGFloat(options.maxSize) / CGFloat(srcData.count))
            try compressGIF(
                inputURL: inputURL, outputURL: outputURL, quality: options.quality, width: width)
            return
        } else {
    ...
    public static func compressGIF(inputURL: URL, outputURL: URL, quality: CGFloat, width: CGFloat)
        throws
    {
        ...
        guard
            let destination = CGImageDestinationCreateWithURL(
                outputURL as CFURL, kUTTypeGIF, CGImageSourceGetCount(source), nil)
        else {
            throw CocoaError.error(.fileWriteFileExists)
        }

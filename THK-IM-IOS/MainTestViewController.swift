...
class MainTestViewController: UIViewController, PerformanceMonitorDelegate {
    ...
    override func viewDidLoad() {
        ...
        textView.frame = CGRectMake(
            self.view.safeAreaInsets.left, self.view.safeAreaInsets.top + 100,
            ...
            .subscribe(onNext: { [weak self] data in
                ...
                if self?.navigationController != nil {
                    ...
                    self?.navigationController?.pushViewController(
                        sessionViewController, animated: true)
                }
            }).disposed(by: self.disposeBag)
        self.view.addSubview(textView)
        let textView1 = UIButton(type: .custom)
        textView1.frame = CGRectMake(
            self.view.safeAreaInsets.left, self.view.safeAreaInsets.top + 200,
            self.view.frame.size.width, 60)
        ...
        textView5.frame = CGRectMake(
            self.view.safeAreaInsets.left, self.view.safeAreaInsets.top + 300,
            self.view.frame.size.width, 60)
        ...
        textView6.frame = CGRectMake(
            self.view.safeAreaInsets.left, self.view.safeAreaInsets.top + 400,
            self.view.frame.size.width, 60)
            ...
            .subscribe(onNext: {
                do {
                    let sessions = try IMCoreManager.shared.database
                        .sessionDao().findByParentId(
                            0, 1_708_851_739_664_322_560, IMCoreManager.shared.severTime)
                    if sessions != nil {
                        for session in sessions! {
                            print("\(session.id) \(session.entityId)")
                            let count = try IMCoreManager.shared.database.messageDao()

    ...
    override func viewDidLoad() {
        ...
        textView2.rx.tap
            .subscribe(onNext: { data in
                ...
                if !self.isPlaying {
                    let filePath = NSTemporaryDirectory() + "temp.oga"
                    let res = OggOpusAudioPlayer.shared.startPlaying(filePath) {
                        db, duration, path, stopped in
                        DDLogDebug(
                            "playing: callback \(db), \(duration) \(path), \(stopped),  \(Thread.current)  "
                        )
                        if stopped {
                            DispatchQueue.main.async {
                                textView2.setTitle("播放", for: .normal)

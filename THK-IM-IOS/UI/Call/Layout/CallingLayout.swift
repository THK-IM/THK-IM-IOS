...
class CallingLayout: UIView {
    ...
    private let switchMicro: UIButton = {
        ...
        v.setImage(UIImage.init(named: "ic_micro_on")?.scaledToSize(CGSize(width: 36, height: 36)), for: .normal)
        v.setImage(UIImage.init(named: "ic_micro_off")?.scaledToSize(CGSize(width: 36, height: 36)), for: .selected)
        v.isSelected = false
    ...
    private let switchSpeaker: UIButton = {
        let v = UIButton(frame: CGRect(x: 0, y: 0, width: 60, height: 60))
        v.setImage(UIImage.init(named: "ic_speaker_on")?.scaledToSize(CGSize(width: 36, height: 36)), for: .normal)
        v.setImage(UIImage.init(named: "ic_speaker_off")?.scaledToSize(CGSize(width: 36, height: 36)), for: .selected)
        v.isSelected = false
        v.setBackgroundImage(Bubble().drawRectWithRoundedCorner(
            radius: 30, borderWidth: 0,
            backgroundColor: UIColor.init(hex: "#80ffffff"), borderColor: UIColor.init(hex: "#80ffffff"),
    ...
    private let switchCameraView: UIButton = {
        ...
        v.setImage(UIImage.init(named: "ic_switch_camera")?.scaledToSize(CGSize(width: 24, height: 24)), for: .normal)
        v.isSelected = false
        v.setBackgroundImage(Bubble().drawRectWithRoundedCorner(
    ...
    private func setupUI() {
        ...
        .subscribe(onNext: { [weak self] in
            ...
            sf.switchMicro.isSelected = liveProtocol.isLocalAudioMuted()
        ...
        self.switchSpeaker.rx.tap.asObservable()
            .subscribe(onNext: { [weak self] in
                ...
                sf.switchSpeaker.isSelected = liveProtocol.isSpeakerMuted()
            }).disposed(by: self.disposeBag)

...
class WelcomeViewController: BaseViewController {
    ...
    private func showLoginUI() {
        ...
        let loginImage = Bubble().drawRectWithRoundedCorner(
            ...
            backgroundColor: UIColor.init(hex: "FF00FFFF"),
            borderColor: UIColor.init(hex: "FF00FFFF"),
            width: 60, height: 60)
        let loginPressedImage = Bubble().drawRectWithRoundedCorner(
            radius: 10.0, borderWidth: 1,
            backgroundColor: UIColor.init(hex: "FF00FFFF", factor: 2),
            borderColor: UIColor.init(hex: "FF00FFFF", factor: 2),
            width: 60, height: 60)
        ...
        let accountRegisterImage = Bubble().drawRectWithRoundedCorner(
            radius: 10.0, borderWidth: 1,
            backgroundColor: UIColor.init(hex: "FF000000"),
            borderColor: UIColor.init(hex: "FF000000"),
            width: 60, height: 60)
        let accountRegisterPressedImage = Bubble().drawRectWithRoundedCorner(
            radius: 10.0, borderWidth: 1,
            backgroundColor: UIColor.init(hex: "FF000000", factor: 2),
            borderColor: UIColor.init(hex: "FF000000", factor: 2),
            width: 60, height: 60)
        ...
        let quickRegisterImage = Bubble().drawRectWithRoundedCorner(
            radius: 10.0, borderWidth: 1,
            backgroundColor: UIColor.init(hex: "FFFF0000"),
            borderColor: UIColor.init(hex: "FFFF0000"),
            width: 60, height: 60)
        let quickRegisterPressedImage = Bubble().drawRectWithRoundedCorner(
            radius: 10.0, borderWidth: 1,
            backgroundColor: UIColor.init(hex: "FFFF0000", factor: 2),
            borderColor: UIColor.init(hex: "FFFF0000", factor: 2),
            width: 60, height: 60)
    ...
    func quickRegister() {
        ...
        DataRepository.shared.userApi.rx.request(.register(registerReq))
            ...
            .subscribe(
                onNext: { [weak self] registerVo in
